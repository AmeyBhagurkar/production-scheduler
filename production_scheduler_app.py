...
# 1. Load HF Token
# ─────────────────────────────────────────────
# Works locally (hf_token.txt) AND on Streamlit Cloud (Secrets Manager)
try:
...
if not HF_TOKEN or HF_TOKEN == "paste_your_token_here":
    st.error("❌ Token not found. Add it to Streamlit Secrets or hf_token.txt locally.")
    st.stop()
...
with tab3:
    ...
    _ac = st.session_state.controller
    alerts = list(_ac.alerts)
    st.caption(f"Total alerts received: **{len(alerts)}** · Auto-updates every 10 seconds")
    if alerts:
        for alert in reversed(alerts):
            st.warning(f"**{alert['Time']}** — {alert['Alert']}")
    else:
        st.success("✅ No alerts. Everything is running smoothly.")
    # Auto-refresh every 10 seconds so alerts appear without ending production
    if _ac.production_started and not _ac.production_ended:
        time.sleep(10)
        st.rerun()
